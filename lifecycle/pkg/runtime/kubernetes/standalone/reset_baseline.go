...
func resetModeState(data []byte, destroy bool) (*resetState, error) {
	...
	if baseline.Mode != ModeStandalone || baseline.Target != "" {
		return nil, fmt.Errorf("finish mode conversion before removing this control plane")
	}
	return &resetState{
...
func resetBootstrapState(destroy bool) (*resetState, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("standalone reset requires a managed baseline or bootstrap journal: %w", err)
	}
	...
	if bootstrap.ClusterID != 0 {
		api, _ := init["localAPIEndpoint"].(map[string]interface{})
...
func verifyResetCleanup() error {
	for _, path := range []string{manifestDir, "/etc/kubernetes/pki", "/var/lib/kubelet"} {
		...
		if len(entries) != 0 {
			return fmt.Errorf("kubeadm cleanup left files in %s; repair cleanup and repeat reset", path)
		}
	}
	for _, name := range []string{"admin.conf", "super-admin.conf", "kubelet.conf", "bootstrap-kubelet.conf", "controller-manager.conf", "scheduler.conf"} {
		if _, err := os.Lstat(filepath.Join("/etc/kubernetes", name)); !os.IsNotExist(err) {
			return fmt.Errorf("kubeadm cleanup did not remove %s: %v", name, err)
		}
	}
	return nil
}
